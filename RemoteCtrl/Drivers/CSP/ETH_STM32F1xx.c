...
static ETH_DMADescTypeDef dma_rx_dscr_tab[ETH_RX_BUF_SIZE];
/**< ethernet tx dma descriptors */
static ETH_DMADescTypeDef dma_tx_dscr_tab[ETH_TX_BUF_SIZE];
...
uint8_t eth_init(uint8_t mac[6]) {
...
#if ETH_MII
    eth_handle.Init.MediaInterface = ETH_MEDIA_INTERFACE_MII;
#elif ETH_RMII
    ...
    eth_handle.Init.DuplexMode = ETH_MODE_FULLDUPLEX;
    eth_handle.Init.PhyAddress = ETH_PHY_ADDRESS;
    eth_handle.TxDesc = dma_tx_dscr_tab;
    eth_handle.RxDesc = dma_rx_dscr_tab;
...
void HAL_ETH_MspInit(ETH_HandleTypeDef *heth) {
    ...
    GPIO_InitTypeDef gpio_init_struct = {.Pull = GPIO_NOPULL,
...
uint32_t eth_read_phy(uint16_t reg) {
    ...
    HAL_ETH_ReadPHYRegister(&eth_handle, reg, &regval);
    return regval;
}
...
void eth_write_phy(uint16_t reg, uint16_t value) {
    ...
    HAL_ETH_WritePHYRegister(&eth_handle, reg, temp);
}
